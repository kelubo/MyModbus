...
SET SERVICE_NAME=ModbusSensorReaderPerl
SET EXECUTABLE=%~dp0modbus_sensor_reader.pl
SET DISPLAY_NAME=Modbus温湿度传感器读取服务(Perl)
SET DESCRIPTION=通过RS485接口读取Modbus温湿度传感器数据(Perl版本)
...
perl -v >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    echo [错误] Perl未找到。请确保Perl已安装并添加到PATH
    pause
    exit /b 1
)
...
REM 安装服务
echo 正在安装服务...
nssm install %SERVICE_NAME% "perl"
